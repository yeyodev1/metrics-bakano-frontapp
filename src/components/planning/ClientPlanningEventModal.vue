    ...
    <div v-if="show" class="planning-modal">
      ...
      <div class="planning-modal__container">
        ...
        <div class="planning-modal__hint">
          ...
          <span>Actividades programadas por la agencia para <strong>{{ workspaceName || 'tu cuenta' }}</strong>.</span>
        </div>

        <!-- Read-only view -->
        <div v-if="entry" class="planning-modal__readonly">
          ...
          <!-- Video planning CTA: Only available for clients -->
          <div class="planning-modal__video-cta" @click="goToVideoPlanning">
            <div class="planning-modal__video-cta-icon">
              <i class="fa-solid fa-clapperboard" />
            </div>
            <div class="planning-modal__video-cta-text">
              <span class="planning-modal__video-cta-label">Planificación</span>
              <span class="planning-modal__video-cta-sublabel">Aprueba y visualiza la planificación para ti</span>
            </div>
        ...
        <div v-else class="planning-modal__readonly">
          <div class="planning-modal__readonly-field">
            <span class="planning-modal__readonly-value">No hay detalles disponibles.</span>
          </div>
          <div class="planning-modal__footer">
            <div class="planning-modal__footer-spacer" />
            <button type="button" class="planning-modal__btn-ghost" @click="emit('close')">
...
.planning-modal {
  ...
  &__container {
    position: relative; background: $white; width: 100%; max-width: 580px;
    border-radius: 24px; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); overflow: hidden;
    max-height: 92dvh; display: flex; flex-direction: column;
    animation: slideUp 0.4s cubic-bezier(0.16, 1, 0.3, 1);
    @media (max-width: 480px) { border-radius: 18px; max-height: 95dvh; }
  }

  &__header {
    padding: 1.5rem 2rem; background: linear-gradient(to bottom, rgba($primary, 0.03), transparent);
    display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba($primary-dark, 0.05);
    @media (max-width: 480px) { padding: 1.25rem 1.5rem; }
  }
  ...
  &__close-btn {
    width: 34px; height: 34px; border-radius: 50%; border: none; background: rgba($primary-dark, 0.05); color: $text-secondary;
    display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.2s;
    &:hover { background: $alert-error-bg; color: $alert-error; transform: rotate(90deg); }
  }

  &__ws-banner {
    display: flex; align-items: center; gap: 1rem; padding: 0.75rem 2rem; background: rgba($primary, 0.02);
  ...
  &__ws-avatar {
    width: 42px; height: 42px; border-radius: 10px; background: linear-gradient(135deg, $primary 0%, darken($primary, 10%) 100%);
    color: $white; font-size: 0.9rem; font-weight: 800; display: flex; align-items: center; justify-content: center;
  }
  ...
  &__readonly-field {
    display: flex; flex-direction: column; gap: 0.4rem; padding: 1rem 2rem; border-bottom: 1px solid rgba($primary-dark, 0.04);
  ...
  &__readonly-label {
    font-size: 0.72rem; font-weight: 800; text-transform: uppercase; letter-spacing: 0.04em; color: $text-secondary; opacity: 0.65;
    display: flex; align-items: center; gap: 0.4rem; i { font-size: 0.7rem; }
  }
  ...
  &__readonly-assignees { display: flex; flex-wrap: wrap; gap: 0.5rem; margin-top: 0.25rem; }
  &__readonly-chip {
    display: flex; align-items: center; gap: 0.5rem; padding: 0.4rem 0.8rem 0.4rem 0.4rem; background: rgba($primary, 0.06); border-radius: 30px;
  ...
  &__video-cta {
    margin: 0.75rem 2rem 0; display: flex; align-items: center; gap: 1rem; padding: 1rem 1.25rem;
    background: linear-gradient(135deg, rgba($primary, 0.06) 0%, rgba($primary, 0.1) 100%);
    border: 1.5px solid rgba($primary, 0.15); border-radius: 16px; cursor: pointer; transition: all 0.2s;
    &:hover { background: linear-gradient(135deg, rgba($primary, 0.1) 0%, rgba($primary, 0.16) 100%); border-color: rgba($primary, 0.3); transform: translateY(-1px); box-shadow: 0 6px 16px rgba($primary, 0.12); }
    @media (max-width: 480px) { margin: 0.75rem 1.25rem 0; }
  }

  &__video-cta-icon {
    width: 44px; height: 44px; border-radius: 12px; flex-shrink: 0; background: linear-gradient(135deg, $primary 0%, darken($primary, 10%) 100%);
    display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 12px rgba($primary, 0.3);
    i { font-size: 1.1rem; color: $white; }
  }

  &__video-cta-text { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 0.15rem; }
  &__video-cta-label { font-size: 0.95rem; font-weight: 800; color: $primary-dark; }
  &__video-cta-sublabel { font-size: 0.72rem; color: $text-secondary; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  &__video-cta-arrow { font-size: 0.75rem; color: $primary; opacity: 0.6; flex-shrink: 0; }

  &__footer {
    padding: 1.25rem 2rem 1.75rem; border-top: 1px solid rgba($primary-dark, 0.05); background: rgba($primary-dark, 0.015);
    display: flex; align-items: center; gap: 1rem;
    @media (max-width: 480px) { flex-direction: column-reverse; align-items: stretch; padding: 1.25rem; }
  }
  &__footer-spacer { flex: 1; }
  &__btn-ghost {
    background: transparent; border: none; color: $text-secondary; font-weight: 700; padding: 0.75rem 1.25rem; border-radius: 12px;
    cursor: pointer; transition: all 0.2s; &:hover { background: rgba($primary-dark, 0.05); color: $primary-dark; }
  }
}
